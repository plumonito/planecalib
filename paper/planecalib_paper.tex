...
We model our cameras using the well-known pinhole model with radial distortion. The projection function $\pv=\Projection(\xv)$ transforms a point in 3D world space $\xv=[x,y,z]^\top$ to a 2D pixel position $\pv=[u,v]^\top$. It first rigidly transforms the point to the camera reference frame
%
\begin{align}
\xv_c &= \Rm \xv + \tv 
\end{align}
%
where the rotation matrix $\Rm$ and the translation vector $\tv$ are the extrinsic parameters of the camera, \ie its rigid pose. Perspective division is then applied, $\xv_n = \hnorm{\xv}_c$. The distorted point $\xv_d$ is obtained by applying the radial distortion function
%
\begin{align}
...
%
\begin{align}
\homogeneous{\pv} &\propto \Km \homogeneous{\xv}_d
\\
\Km &= \begin{bmatrix}
f_x & 0 & u_0 \\
0 & f_y & v_0 \\
...
The camera model contains 6 degrees of freedom (DoF) for the extrinsic parameters (three for rotation and three for translation) and 6 DoF for the intrinsic parameters (two focal lengths, two for the principal point, and two distortion coefficients). The goal of the calibration algorithm is to recover the intrinsic parameters from a sequence of images of a planar scene.
...
The relation of Eq.~\eqref{eq:homography} only holds for a pinhole camera without distortion. We note that in the case of a camera with distortion, the undistorted measurements will still follow Eq.~\eqref{eq:homography}. We can thus estimate the distortion model by finding the coefficients that allow the measurements to be modelled by a homography. 

The conventional distortion model of Eq.~\eqref{eq:distortion} is applied to the point in metric coordinates, but these are not available in the projective reconstruction stage. Thus, we define an alternative distortion model that is applied directly to pixel coordinates
%
\begin{align}
...
The methods mentioned so far obtain an initial projective reconstruction but it is not optimal. The chosen reference frame introduces a bias to the obtained homographies and the distortion coefficients haven't been estimated. We perform a robust non-linear minimization \cite{ceres-solver} over all parameters to obtain the optimal projective reconstruction. The formulation of the minimization problem is as follows, 
%
\begin{align}
...
\section{Calibration constraints}
...
\\
\av_i^\top \av_i - \bv_i^\top \bv_i &= (\Km^{-1} \Hm_i \Km \av)^\top (\Km^{-1} \Hm_i \Km \av) - (\Km^{-1} \Hm_i \Km \bv)^\top (\Km^{-1} \Hm_i \Km \bv) \notag \\
&= (\Km^{-1} \Hm_i \xv_1)^\top (\Km^{-1} \Hm_i \xv_1) - (\Km^{-1} \Hm_i \xv_2)^\top (\Km^{-1} \Hm_i \xv_1) \notag \\
